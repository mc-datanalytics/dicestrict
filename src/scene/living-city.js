...
function infrastructure() {
  ...
  // Casino destination, no additional board square or access cost.
  g.box([-1.25,.5,-.8],[1.75,.14,1.45],stone,.14);
  tower(g,-1.25,-.95,1.32,.86,.83,'#517e74',true);
  g.cylinder([-1.25,1.45,-.95],.48,.20,gold,22,.43);
  g.cylinder([-1.25,1.61,-.95],.32,.13,ink,20,.24);
  for(let i=0;i<12;i++) {
    const a=i*Math.PI/6;g.material=3;
    g.block([-1.25+Math.cos(a)*.40,1.46,-.95+Math.sin(a)*.40],[.055,.065,.05],i%2?'#e98b7a':'#ffe4a0');g.material=0;
  }
  g.box([-1.25,.85,-.36],[1.48,.17,.33],gold,.04);
  g.block([-1.25,.55,-.02],[.52,.025,.50],'#ba796c');
  for(const x of [-1.88,-.63]) {g.cylinder([x,.69,-.37],.045,.5,stone,10);tree(g,x,-1.56,.65);}
  // Street lamps with gradual illumination, and local fictional advertising.
  for(const [x,z] of [[2.2,.6],[-2.2,.4],[.7,2.2],[-.35,-2.05]]) {
    g.block([x,.89,z],[.035,.98,.035],ink);g.material=2;
    g.sphere([x,1.40,z],.07,gold,8,4);g.material=0;
  }
...
function neonSign(text,w=1.22) {
  const g=new Geometry();
  // Tiny original bitmap lettering, assembled as one mesh, no downloaded fonts/textures.
  const font={C:['111','100','100','100','111'],A:['010','101','111','101','101'],S:['111','100','111','001','111'],I:['111','010','010','010','111'],N:['101','111','111','111','101'],O:['111','101','101','101','111'],R:['110','101','110','101','101'],U:['101','101','101','101','111'],D:['110','101','101','101','110'],E:['111','100','110','100','111']};
  g.block([0,0,0],[w,.27,.035],ink);g.material=3;
  const pixel=w/(text.length*4+1);
  [...text].forEach((c,i)=>(font[c]??font.I).forEach((line,y)=>[...line].forEach((v,x)=>{if(v==='1')g.block([-w/2+(i*4+x+1)*pixel,(2-y)*.038,.025],[pixel*.77,.029,.015],gold);})));g.material=0;
  return g;
}
class LivingCity {
  constructor(renderer) {
    ...
    this.crane=renderer.mesh(crane());this.sign=renderer.mesh(neonSign('CASINO'));this.ad=renderer.mesh(neonSign('AURORA',.80));
  ...
  objects(seconds,rain=0) {
    ...
    // Fictional signage changes facing slowly. No commercial requests or ad-tracking.
    out.push({mesh:this.ad,model:model(1.45,1.03,-.2,0,running?Math.sin(t*.22)*.18:0),noShadow:true});
