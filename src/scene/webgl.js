...
class Renderer {
  ...
  mesh(geo){
    ...
    let indexBuffer=null,indexType=null;
    if(geo.indices){indexBuffer=gl.createBuffer();gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,indexBuffer);gl.bufferData(gl.ELEMENT_ARRAY_BUFFER,geo.indices,gl.STATIC_DRAW);indexType=geo.indices instanceof Uint32Array?gl.UNSIGNED_INT:gl.UNSIGNED_SHORT;}
    const mesh={vao,buffer,indexBuffer,indexType,count:geo.indices?.length??data.length/12,vertices:data.length/12,bytes:data.byteLength+(geo.indices?.byteLength??0),name:geo.name??'legacy-mesh'};this.meshes.add(mesh);this.meshUploads++;return mesh;
  }
  ...
  destroy(){for(const mesh of [...this.meshes])this.drop(mesh);const gl=this.gl;gl.deleteProgram(this.p);gl.deleteProgram(this.dp);gl.deleteTexture(this.atlas);gl.deleteTexture(this.surface);gl.deleteTexture(this.shadow);gl.deleteFramebuffer(this.fb);}
}
