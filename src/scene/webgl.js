...
out vec3 vNormal;out vec3 vColor;out vec2 vUv;out float vTex;out vec4 vShadow;
void main(){vec4 world=uModel*vec4(aPosition,1.);gl_Position=uVP*world;vNormal=normalize(mat3(uModel)*aNormal);vColor=aColor;vUv=aUv;vTex=aTex;vShadow=uLight*world;}`;
const FRAG=`#version 300 es
precision highp float;
in vec3 vNormal;in vec3 vColor;in vec2 vUv;in float vTex;in vec4 vShadow;
uniform sampler2D uAtlas;uniform sampler2D uShadow;uniform float uUseShadow;out vec4 frag;
void main(){vec3 base=vColor; if(vTex>.5){vec4 t=texture(uAtlas,vUv);base=mix(base,t.rgb,t.a);}
...
class Renderer {
  ...
  pass(p,vp,objects){const gl=this.gl;gl.useProgram(p);gl.uniformMatrix4fv(this.uniform(p,'uVP'),false,vp);
    if(p===this.p){gl.uniformMatrix4fv(this.uniform(p,'uLight'),false,this.light);gl.uniform1f(this.uniform(p,'uUseShadow'),this.shadows&&this.shadowOK?1:0);
  ...
  render(vp,objects){const gl=this.gl;
    ...
    gl.bindFramebuffer(gl.FRAMEBUFFER,null);gl.viewport(0,0,this.canvas.width,this.canvas.height);gl.clearColor(.955,.95,.924,1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);this.pass(this.p,vp,objects);
  }
