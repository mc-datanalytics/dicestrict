...
class Harmony {
 ...
 refresh(){
  ...
  const used=new Set(this.entries.map(e=>e.key));for(const [key,item] of [...this.cache].sort((a,b)=>a[1].used-b[1].used)){if(this.cache.size<=HARMONY_CACHE_LIMIT)break;if(!used.has(key)){this.renderer.drop(item.mesh);this.cache.delete(key);}}
 }
...
export {Harmony,HARMONY_SPEC,HARMONY_LOTS,HARMONY_CACHE_LIMIT};
