...
vec3 marinaShade(vec3 color,vec3 normal,float diffuse,float shadow){
 ...
 float denom=NoH*NoH*(a2-1.)+1.;float D=a2/(3.14159*denom*denom+.0001);
 float k=(rough+1.)*(rough+1.)*.125;
 float G=NoV/(NoV*(1.-k)+k)*NoL/(NoL*(1.-k)+k);
 vec3 f0=mix(vec3(.04),albedo,metallic),F=f0+(1.-f0)*pow(1.-VoH,5.);
 vec3 spec=D*G*F/max(4.*NoV*max(NoL,.01),.01);
