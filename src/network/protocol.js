...
function readPacket(raw) {
  ...
  if (p.type === 'action') {
    ...
    const r=p.rules;if(!r||Object.keys(r).length!==5||!validOpening(r.opening)||!Number.isInteger(r.rounds)||r.rounds<4||r.rounds>30||!Number.isInteger(r.mobility)||r.mobility<0||r.mobility>3||typeof r.finishOnBankruptcy!=='boolean'||typeof r.casino!=='boolean')throw Error('Règles de salon invalides.');
