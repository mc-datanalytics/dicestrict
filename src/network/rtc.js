...
class RoomSession {
  constructor({onLobby,onState,onError,onClosed}){
    Object.assign(this,{onLobby,onState,onError,onClosed});this.peers=new Map();this.room=null;this.state=null;this.closed=false;this.paused=false;this.ready=new Set();
  }
  ...
  receive(id,msg,peer){
    ...
    if(this.isHost){
      if(msg.type==='ready'){this.ready.add(id);this.broadcast('roster',{ready:[...this.ready]});this.emitLobby();return;}
      ...
      if(msg.gameId!==this.state.id||msg.revision!==this.state.revision){this.send(id,'snapshot',{state:this.state});return;}
  ...
  start(rounds=12){
    ...
    this.state=createGame(seats,randomSeed(),{rounds,id:`room-${crypto.randomUUID()}`});
  ...
  rematch(rounds=12){this.start(rounds);}
  emitLobby(){if(this.room)this.onLobby({...this.room,isHost:this.isHost,members:this.room.members.map(m=>({...m,ready:this.ready.has(m.id)}))});}
