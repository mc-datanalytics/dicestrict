...
function controls(side,r){$(side+'-controls').innerHTML=`<fieldset id="${side}-fields"><div class="rule-grid"><label>Manches maximum<input id="${side}-rounds" type="number" min="4" max="30" step="1" value="${r.rounds}" required></label><label>Jetons Mobilité<input id="${side}-mobility" type="number" min="0" max="3" step="1" value="${r.mobility}" required></label><label class="wide check"><input id="${side}-bankruptcy" type="checkbox" ${r.finishOnBankruptcy?'checked':''}>Fin commune à la première faillite</label><label class="wide">Casino · crédits de partie<select id="${side}-casino"><option value="off">Désactivé</option><option value="none">Activé · personne ne mise</option><option value="all-20">Tous les bots · 20 crédits / manche</option><option value="all-60">Tous les bots · 60 crédits / manche</option><option value="focal-60">Identité 1 seule · 60 crédits / manche</option></select></label></div></fieldset>`;$(side+'-casino').value=r.casino?r.casinoPolicy:'off';}
function readConfig(){
  const rule=side=>({rounds:Number($(side+'-rounds').value),mobility:Number($(side+'-mobility').value),finishOnBankruptcy:$(side+'-bankruptcy').checked,casino:$(side+'-casino').value!=='off',casinoPolicy:$(side+'-casino').value==='off'?'none':$(side+'-casino').value});
...
function setBusy(busy){ui.busy=busy;for(const id of ['run-fields','baseline-fields','candidate-fields'])$(id).disabled=busy;$('run').disabled=busy;$('stop').hidden=!busy;$('load-replay').disabled=busy||!ui.report;$('export-config').disabled=busy;}
...
function showReplay(replay){
  verifyReplay(replay);ui.replay=replay;$('export-replay').disabled=false;
  ...
  renderReplay();$('status').textContent='Rejeu vérifié : chaque commande et son empreinte correspondent au moteur.';
}
function renderReplay(){
  ...
  $('replay-log').textContent=`${r.side==='candidate'?'B':'A'} · graine ${r.seedIndex}, rotation ${r.rotation} · manche ${s.round} · ${step?s.log[0]?.text:'Ouverture de la partie.'}`;
