...
for(const file of files){const r=spawnSync(process.execPath,['--check',file],{encoding:'utf8'});if(r.status){console.error(r.stderr);process.exit(1);}const source=await readFile(file,'utf8');for(const m of source.matchAll(/(?:from\s*|import\s*\()(['"])(\.[^'"]+)\1/g)){try{await readFile(resolve(dirname(file),m[2]));}catch{throw Error(`Missing import ${m[2]} in ${file}`);}}}
